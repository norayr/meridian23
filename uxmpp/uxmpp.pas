...
uses
  ...
  Windows,
  {$ENDIF}
  Classes, SysUtils, tcpsynapse, xmltag, ExtCtrls, debug;
...
//    procedure SendCommand(XML:string);
  ...
  public
    ...
    procedure Login;
    procedure Logout;
    procedure SendCommand(XML:string);   //here temporarily to send messages from outside
...
begin
  ...
  if Assigned(FOnPresence) then begin
     ...
     //if tag.TagExists('status') then begin
