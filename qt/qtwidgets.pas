    ...
    case QEvent_type(Event) of
      ...
      QEventKeyRelease:
        begin
          ...
          {$IFNDEF MAEMO5} if Result then{$ENDIF}
            Result := SlotKey(Sender, Event) or ((LCLObject <> nil) and (LCLObject is TCustomControl));
        end;
...
{$ifdef TRX}
var  {This is used to make use of the Maemo5 "Symbols Dialog"}
  FKEPress, FKERelease : QKeyEventH;
  txs : WideString;
  txkey : integer;
{$endif}
begin
  ...
  begin
    QInputMethodEvent_commitString(QInputMethodEventH(Event), @txs);
    txkey := 0;
    if Ord(txs[1]) = 9 then
      txkey := 9;
    FKEPress := QKeyEvent_Create(QEventKeyPress,txkey,QtNoModifier,@txs);
    Result := SlotKey(Sender, FKEPress);
    FKERelease := QKeyEvent_Create(QEventKeyRelease,txkey,QtNoModifier,@txs);
    Result := SlotKey(Sender, FKERelease);
  end
  else
  {$endif}
